...
test("renders the selected key details and exactly seven diatonic chord cards", async ({ page }) => {
  ...
  await expect(selectionBar.locator(".key-mode-selection-current-placeholder")).toHaveText("Play a chord to choose inversion and voicing");
  ...
  await expect(selectionBar.locator(".key-mode-chord-inversion-select")).toBeVisible();
  await expect(selectionBar.locator(".key-mode-chord-voicing-select")).toBeVisible();
...
test("shows the expanded triad voicing presets after a key explorer chord is played", async ({ page }) => {
  ...
  await expect(selectionBar.locator(".key-mode-chord-voicing-select")).toHaveValue("close");
  await expect(selectionBar.locator(".key-mode-selection-summary-code")).toHaveCount(0);
  await expect(
    selectionBar.locator(".key-mode-chord-voicing-select option")
...
test("lets you audition a selected inversion from the shared key explorer bar", async ({ page }) => {
  ...
  await page.locator(".key-mode-chord-card").first().locator(".key-mode-chord-play").click();
  await selectionBar.locator(".key-mode-chord-inversion-select").selectOption("1");
...
test("updates the shared key explorer bar when a different chord card is played", async ({ page }) => {
  ...
  await expect(selectionBar.locator(".key-mode-selection-current-name")).toHaveText("Dm");
  await expect(selectionBar.locator(".key-mode-chord-inversion-select")).toHaveValue("0");
  await expect(selectionBar.locator(".key-mode-chord-voicing-select")).toHaveValue("close");
...
test("shows selected key explorer voicing details on the progression block", async ({ page }) => {
  ...
  await selectionBar.locator(".key-mode-chord-inversion-select").selectOption("2");
  await selectionBar.locator(".key-mode-chord-voicing-select").selectOption("close-high");
